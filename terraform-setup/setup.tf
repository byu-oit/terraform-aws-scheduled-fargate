terraform {
  ...
  backend "s3" {
    ...
    key            = "terraform-aws-scheduled-fargate/setup.tfstate"
    region         = "us-west-2"
  }
  required_providers {
    aws = {
...
locals {
  name    = "terraform-scheduled-fargate"
  gh_org  = "byu-oit"
  gh_repo = "terraform-aws-scheduled-fargate"
  env     = "dev"
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      repo                   = "https://github.com/byu-oit/terraform-aws-scheduled-fargate"
